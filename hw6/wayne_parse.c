    ...
    while(argc < MAX_ARGS-1 && sc->numCommands < MAX_COMMANDS)
    {
        ...
        switch(*line)
        {
            ...
            case '#':
                *line = '\0';
                *(line+1) ='\0';
                *(line+2) = '\0';//not necessary but just to be safe
                break;
                /*
            ...
            case '\'':
                *line++ = '\0';
                sc->argv[argc++] = line;
                while(*line != '\'')  /* find the end of the argument */
                    ++line;
                *line = '\0';
                is_single=1;
                break;
        ...
        while(*line && !isSeparator(*line))  /* find the end of the argument */
            ++line;
        if(*line == '\0' && is_single ==0)        /* end-of-line, we're done */
            break;
...
void Print_command(struct commandLine cmdLine)
{
